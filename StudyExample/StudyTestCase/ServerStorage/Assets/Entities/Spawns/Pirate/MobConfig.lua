return {
	-- General
	Name = "Pirate", -- Name of the mob shown in its interface
	Rank = nil, -- Mob Rank - 'nil' for no rank, or a string for a rank ("Boss")
	
	Level = {5, 1}, -- Mob Level, followed by Min Level (req to kill/access the mob - this is typically the realm level / boss portal level)
	MobTypes = {"Regular"}, -- Weapons can use these values to change damage modifier versus mobs, they'll do more damage to these!
	
	-- Visuals
	Color = Color3.fromRGB(255, 97, 97), -- Color for mob text & display on player hud, set to nil to use default

	HealthBarColor = nil, -- Set to use custom healthbar color
	HighlightColor = nil, -- Set to use custom damage highlight color
	
	RandomizeAppearance = nil, -- Set to nil to not use, else, choses a random mob from Entities --> Alternatives from the given names to replace with (and current model)
	...
	InstantRefreshData = {
		CanRefresh = true, -- The player who engaged the mob (which no other players can engage) will regenerate when player is dead or far enough away
	...
	BossData = {
		Type = "AtDistance", -- 'AtDistance', 'DamageDealt', or 'None', Based on how the HUD will be shown. DamageDealt = if hit, AtDistance = distance required [2]
		Distance = 75, -- Distance until both shown (if AtDistance) & left
	...
	AIData = {
		...
		LookAtTarget = false, -- Looks at the player the mob is targeting (similar to melee mob align)
	...
	Health = 45, -- Amount of max health the mob spawns with
	Defense = {2, true}, -- The amount of damage the mob resists from hits
	...
	-- Projectile
	Projectile = "Cannonball", -- Name of projectile in RS --> Assets --> Projectiles

	Cooldown = 4, -- Time until projectile can be shot again
	ActivateSound = 18425374322, -- Sound that plays when projectile is shot (can use strings)

	ProjectileData = {
		Velocity = 350, -- Speed of projectile
		Acceleration = Vector3.new(0, -workspace.Gravity/9.5, 0), -- Acceleration of projectile
		SpreadAngle = NumberRange.new(0, 2), -- Spread angle of projectile (in degrees)
		Pierce = 1, -- Amount of entities that can be caught by single projetile
	},
	
	-- Behavior
	Damage = 25, -- Damage dealt to any player that touches the mob
	...
	RespawnRadius = nil, -- Random stud radius that the mob can spawn in, calculated off of floor position (set to nil to disable)
	...
	AttackCycle = true, -- Set AttackCycles in ReplicatedStorage --> Modules --> Entity --> attackFunctions --> (Name or "Default")
	HitCycle = false, -- Set HitCycles in ReplicatedStorage --> Modules --> Entity --> hitCycleFunctions --> (Name or "Default")
	-- * For both of these, set it to a string, (ie. 'Default') to use the module w/ that name. Setting to true uses the mob's name.

	MaxDamageCap = 0.333, -- Max percent (decimal) of Mob's MaxHealth can be dealt by the player, set to nil to not use. ie. 0.33 --> 33k DMG out of 100k HP can be max dealt
	...
	-- * Mob drops that can be given to the player on death, like stats (Level / XP / Gold) & items (Armor / Tool)
	...
	CustomAnimations = { -- If any values below are nil, they use the animation defaults, based off the default Thief rig.
		...
		Fire = nil,  -- Animation id for Fire animation (ie. custom rigs) - keep to nil for default anims
